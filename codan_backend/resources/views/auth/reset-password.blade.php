...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="h-full">
    <head>
        ...
        <title>Atur Ulang Sandi | CODean Marketplace</title>
    ...
    <body class="h-full bg-slate-950 overflow-hidden">
        ...
        <div class="relative z-10 h-full flex flex-col p-8 lg:p-16" style="zoom: 0.8;">
            ...
            <header class="flex justify-between items-center mb-auto shrink-0">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-white rounded-xl flex items-center justify-center">
                        <i data-lucide="shield-check" class="text-[#00f2ea] w-6 h-6"></i>
                    </div>
            ...
            <div class="flex-1 flex items-center justify-center">
                ...
                <!-- Reset Box Centered -->
                <div class="w-full max-w-lg animate-in zoom-in duration-500">
                    <div class="cyan-reset-box p-10 lg:p-14">
                        <h1 class="text-center text-3xl font-black text-slate-900 mb-10 tracking-tight uppercase leading-none">UPDATE YOUR <br> PASSWORD</h1>
                        ...
                        <form method="POST" action="{{ route('password.store') }}" class="space-y-4">
                            ...
                            <div class="space-y-1.5">
                                <label for="email" class="text-slate-900 font-black text-xs ml-2 uppercase tracking-widest">Confirm Email :</label>
                                <input id="email" type="email" name="email" value="{{ old('email', $request->email) }}" required class="custom-input">
                                <x-input-error :messages="$errors->get('email')" class="mt-1" />
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    <label for="password" class="text-slate-900 font-black text-xs ml-2 uppercase tracking-widest">New Password :</label>
                                    <input id="password" type="password" name="password" required class="custom-input">
                                </div>
                                <div class="space-y-1.5">
                                    <label for="password_confirmation" class="text-slate-900 font-black text-xs ml-2 uppercase tracking-widest">Confirm :</label>
                                    <input id="password_confirmation" type="password" name="password_confirmation" required class="custom-input">
                                </div>
                            ...
                            <div class="pt-10 flex flex-col items-center lg:items-end gap-8">
                                <button type="submit" class="btn-update uppercase tracking-widest text-lg w-full lg:w-auto shadow-2xl">
                                    UPDATE NOW
                                </button>
                            </div>
                        </form>
